...
const cacheCapacity int = 300 * 1024

func TestCache(t *testing.T) {
	tc := New(DefaultExpiration, 0, cacheCapacity)
...
func TestCacheTimes(t *testing.T) {
	...
	tc := New(50*time.Millisecond, 1*time.Millisecond, cacheCapacity)
	tc.Set("a", 1, DefaultExpiration)
...
func TestNewFrom(t *testing.T) {
	...
	tc := NewFrom(DefaultExpiration, 0, m, cacheCapacity)
	a, found := tc.Get("a")
	if !found {
...
func testFillAndSerialize(t *testing.T, tc *Cache) {
	...
	oc := New(DefaultExpiration, 0, cacheCapacity)
	err = oc.Load(fp)
	if err != nil {
